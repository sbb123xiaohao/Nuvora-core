...
x64 的 `GROW` 仍是**立即逐页分配**：请求失败会回滚全部页表与页，已提交的用户 break 不变。用户堆虚拟区从 `0x50000000` 到 `0x70000000`，最多 512 MiB，受可用 RAM 和同一 1–2 GiB 用户地址窗口限制。当前 x64 普通文件上限 4 MiB；16 MiB 快照槽与磁盘容量不会自动变成训练数据集或交换空间。

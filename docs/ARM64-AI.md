...
| 内存 | 启动内存图；最多管理 64 GiB RAM；单进程按页申请 512 MiB 用户堆 | 读取 DTB 内存和保留范围；最多管理 64 GiB；4 KiB 所有权位图与连续页申请 |
...
x64 的 `GROW` 仍是**立即逐页分配**：请求失败会回滚全部页表与页，已提交的用户 break 不变。用户堆虚拟区从 `0x50000000` 到 `0x70000000`，最多 512 MiB，受可用 RAM 和同一 1–2 GiB 用户地址窗口限制。普通文件仍限 128 KiB；16 MiB 快照槽与磁盘容量不会自动变成训练数据集或交换空间。
...
ARM64 在 64、256、1024、5120 MiB 三种 RAM 配置下验证设备树、页表、1 MiB 缓冲清零与归还、EL0 NEON、SVC 和隔离，共每轮 10 个断言。x64 的内存阶段覆盖 32 MiB 至 5 GiB，实际检查 64 MiB 与 256 MiB 模型缓冲申请、逐页读写、回收及低内存失败路径。所有这些验证使用 QEMU TCG；64 GiB 是分配器配置的上限，没有进行 64 GiB 实机压力认证。

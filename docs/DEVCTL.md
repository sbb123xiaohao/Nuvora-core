...
只接受已经分配的内存 BAR，拒绝 I/O、空 BAR、64 位 BAR 的高半槽、无效编码和未分配资源。首地址必须按页对齐，并处于 CPU 和当前架构可寻址的范围；i686 不能映射 4 GiB 以上地址。
...
`forge probe devctl` 只运行设备控制回归；`trial` 包含这组检查。GPU 可映射时，x64 有 14 项、i686 有 13 项新增断言，覆盖未知操作、预留操作、错误指针、输出只读/跨页/溢出、索引、清零错误响应、连续 600 次不完整输出请求、连续 600 次重复映射和用户态 MMIO 访问隔离。

...
7. **用户环境与桌面**：x64 已有开放 SDK、UEFI 像素矩形接口和键盘文件浏览器；下一步实现 PS/2/USB 鼠标与触控输入、用户态进程间通信、桌面合成服务和窗口协议，然后是 Unicode 字体、剪贴板、无障碍、成熟运行库、动态链接、用户/组权限、配额、程序安装工具，以及是否实现 POSIX 兼容层的明确决策。已实现范围见 [SDK.md](SDK.md)。

...
128 MiB 镜像可创建两个分区。默认 512 MiB 单分区镜像使用双份
128 MiB 快照；较小分区使用 16 MiB 槽。现有镜像不会被覆盖或自动改写。要重新创建
...
64 MiB，新建足够大的分区每份快照最多 128 MiB，所有卷共用
512 个文件节点。旧盘沿用盘头记载的 1/16 MiB 槽位；扩容已有
镜像不会改变槽位几何。

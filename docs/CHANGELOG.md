...
- 新盘默认 NVSTORE3，文件内容按 4 KiB 区间写入全卷数据区；两个 2 MiB
  元数据根保留提交历史。64 位文件长度/偏移、稀疏空洞、COW 块保护、
  随机覆盖合并、提交失败冻结和校验回退；追加 SEEK64/STAT64/LIST64。
- 新建镜像默认 8 GiB，可按实际磁盘需求选择容量。导入器流式复制大文件，
  另存迁移工具保留旧单卷镜像，旧格式继续兼容。
- MPEG 改为文件回调流式读取，去掉 640×480 额外限制。加入 FLAC、独立
  MP2、PCM 8/16/24/32-bit、float WAV、RF64、多声道混音和采样率转换。
  提供宿主 FFmpeg 转换/验证/导入工具；不宣称原生 MP4/H.264/AAC。
- 桌面和 Loom 使用 64 位大小；文件列表覆盖当前全部节点容量，Media
  浏览时不再因为前 128 项是其他文件而漏掉后续媒体。修复跨持久/临时卷
  重命名导致底层快照引用失效，以及非驻留文件误作 ELF 指针的问题。
- x64 构建及 19 组宿主回归通过（C 夹具 UBSan）；10 GiB 稀疏文件、
  160 MiB 实际连续写入、>4 GiB 物理位置、随机覆盖/OOM/磁盘满/损坏恢复、
  宿主与内核互操作、旧盘迁移、FLAC/720p MPEG/RF64 已验证。
  无本轮 QEMU 或实体硬件运行结论。详见 STORAGE-0.10.md。

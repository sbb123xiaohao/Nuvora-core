...
static void fb_cell(u32 index, u16 cell, bool invert) {
    ...
    if ((cell & 255) < 32 || (cell & 255) > 126)
        return;
    const u8 *glyph = nv_font5x7[(cell & 255) - 32];
...
int console_display_info(struct nv_display_info *out) {
...
int console_display_acquire(u32 pid) {
    if (!fb_ready) return -NV_ENODEV;
    if (screen_owner == pid)
        return graphics_owner ? 0 : -NV_EBUSY;
...
int console_display_present(u32 pid, const struct nv_display_present *r) {
    if (!fb_ready) return -NV_ENODEV;
    if (screen_owner != pid || !graphics_owner) return -NV_EACCESS;
    if (!r->width || !r->height || r->x >= fb_width || r->y >= fb_height ||
        r->width > fb_width - r->x || r->height > fb_height - r->y ||
...
int display_ioctl(u32 op, u32 pointer) {
    ...
    if (op == NV_DISPLAY_INFO) {
        if (!user_range(current->pd, pointer, sizeof(struct nv_display_info), true))
            return -NV_EFAULT;
        struct nv_display_info info;
        int r = console_display_info(&info);
